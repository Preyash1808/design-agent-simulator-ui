...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const r = await fetch(`${api}/api/metrics_public?run_id=${encodeURIComponent(runId)}`, {
    ...
    return new Response(JSON.stringify({ error: 'timeout' }), { status: 200, headers: { 'Content-Type': 'application/json; charset=utf-8', 'x-backend-timeout': '1' } });
  } finally {
